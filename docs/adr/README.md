...
| [0065](./0065-autorisation-decidee-avant-la-lecture-du-corps.md) | L'autorisation d'une méthode de contrôleur est évaluée AVANT la liaison et la validation du corps : un appelant sans habilitation recevait 400 au lieu de 403 selon la charge utile envoyée — un point d'entrée fermé devenu oracle de validation. La règle n'est pas recopiée dans la chaîne de filtres (seconde source de vérité) mais lue plus tôt ; le pré-contrôle ne sait que refuser, jamais autoriser (§11.1, OWASP A01) | Accepté |
| [0066](./0066-le-cycle-apqp-appartient-au-tenant.md) | Le cycle APQP cesse d'être une constante du front : ses phases et leurs livrables deviennent des données du tenant (V124), amorcées par le manuel AIAG à la première lecture puis remaniables — renommer, retirer, ajouter, réordonner. Le rang du jalon dans le V est calculé par le serveur (`min(i, n+1-i)`), de sorte que le schéma tienne quel que soit le nombre de phases ; réorganiser porte sur le cycle entier (422 sur un ordre partiel) et n'est ouvert qu'au pilotage qualité (§5, §16, §22.11) | Accepté |
| [0067](./0067-la-boite-a-idees-partage-les-lignes-du-cercle.md) | La boîte à idées partage les lignes du cercle : un seul objet (`circle_proposals`), deux façades HTTP, plutôt qu'une table dupliquée ; `tenant_id` porté par la ligne elle-même (V125) pour qu'une idée sans cercle reste rattachée à un client ; la fenêtre de vote se ferme à la décision et le décompte n'est jamais stocké ; l'acteur d'une proposition (`proposedBy`, `validatedBy`) vient désormais du jeton des deux côtés, corrigeant une garde qui comparait deux valeurs écrites par le même appelant ; déposer et voter ouverts à tout authentifié, arbitrer réservé au pilotage qualité (§3.6, §16, §18.2) | Accepté |
| [0068](./0068-un-livrable-apqp-se-coche-et-se-prouve.md) | Un livrable APQP cesse d'être un libellé : il se coche (heure du serveur, acteur du jeton, décocher efface les deux), se prouve par des pièces jointes — `.docx` en tête de liste blanche, 5 par livrable, 50 Mo par tenant, sur le motif éprouvé des preuves PDCA — et porte un GENRE fermé (`ATTACHMENT`, `MODULE_LINK`, `DATA_ENTRY`, `CHECKLIST`) qui décide de ce que son formulaire demande, plutôt qu'un formulaire codé par livrable ; un `data jsonb` unique validé par genre (422 sinon), un renvoi vérifié dans le tenant (un lien mort affirme une preuve qui n'existe pas), et un dossier PPAP qui AGRÈGE le cycle au lieu de tenir une seconde liste, son compte calculé par le serveur (V126, V127, §3.6, §22.11) | Accepté |
| [0069](./0069-une-reclamation-externe-se-rejette.md) | Une réclamation externe jugée non fondée se REJETTE, avec motif obligatoire dans sa propre colonne : annuler dit « ce constat n'avait pas lieu d'être », rejeter dit « il a été instruit, voici pourquoi » — et c'est la seconde phrase que le client et l'auditeur viennent lire. Statut réel et non libellé (un renommage de tuile aurait menti sur la donnée exportée), réservé à l'origine EXTERNAL par le SERVICE et non par l'écran, terminal comme `CLOSED`, et tuile « Rejetées » sur le seul écran externe (V128, §4.3, §4.9) | Accepté |
| [0070](./0070-le-referentiel-apqp-suit-la-langue.md) | Le référentiel APQP **suit la langue** : révise l'ADR 0068 §8, qui confondait à qui appartient une ligne et dans quelle langue elle se lit. Chaque ligne d'amorçage porte sa clé (V129) ; le texte suit la langue demandée à trois conditions cumulées — clé connue, ligne issue du référentiel, et personne ne l'a retouchée (`updatedAt == createdAt`). Dès qu'un utilisateur reformule un libellé, sa formulation gagne définitivement dans toutes les langues : on ne traduit pas ce qu'un utilisateur a écrit. La langue vient du BUILD (`LOCALE_ID` → `Accept-Language`) et non du navigateur, et la fidélité normative survit dans la traduction — « Control plan » → « Plan de surveillance », sigles conservés (82 entrées × 6 langues, §15.1, §22.7) | Accepté |
| [0071](./0071-le-rapport-8d-agrege-et-se-scelle-a-la-cloture.md) | Le rapport **8D** d'une non-conformité **agrège** plutôt qu'il ne recopie : cinq disciplines sur huit n'ont aucune colonne et relisent la NC, les Ishikawa et 5 pourquoi rattachés, la CAPA escaladée avec ses preuves, le PFMEA et les plans de surveillance (même parti que le dossier PPAP, ADR 0068) ; seules D1, D3 et D8 se saisissent, et leur absence est ÉCRITE — le rapport se déclare « partiel » plutôt que d'afficher des cases vides, y compris pour dire que le Poka-Yoke ne se rattache qu'à un projet DMAIC. À la clôture, l'émission FIGE le texte des huit disciplines (V130), signe l'empreinte du PDF (Ed25519 + ML-DSA-65) et l'ancre ; le rendu est déterministe (`/ID` dérivé du contenu) et le téléchargement refuse un document qui ne retombe plus sur son empreinte. Lire est ouvert, émettre exige le pilotage qualité ET un second facteur (§3.6, §11.4, §18.2 #5) | Accepté |
| [0072](./0072-un-projet-apqp-et-un-seul-formulaire-de-livrable.md) | L'APQP se mène par **PROJET** — comme le PFMEA, chacun avec son cycle en V et son propre dossier PPAP (`NPI`, `ToW`, `New customer`, `Other`) : un cycle unique par tenant mélangeait des programmes qui se remettent séparément. Et **tout livrable porte le même formulaire**, celui du classeur du client (artefact attendu, PPAP requis, responsable, échéance, statut, avancement, notes) plus une pièce jointe TOUJOURS : révise l'ADR 0068 §1 et §2, dont le GENRE fermé rendait la moitié des livrables non cochables et faisait répondre quatre écrans à une seule question. Le renvoi vers une AMDEC / un PDCA / une CAPA devient un champ facultatif, toujours vérifié dans le tenant. **La case pilote** : cocher pose « terminé » et 100 %, décocher les ramène en arrière — une seule vérité. Le référentiel est réconcilié avec le classeur (48 artefacts × 6 langues, « Control plan » portant deux artefacts pour ses deux états), et la V131 verse les cycles existants dans un « Projet par défaut » sans rien perdre — cases, marques, pièces et contenu saisi replié dans les notes (§3.6, §5.2, §15.1, §18.2 #2 et #9) | Accepté |
| [0073](./0073-une-verification-capa-s-exige-avant-de-se-constater.md) | Une vérification d'efficacité CAPA **s'exige avant de se constater** : le dossier portait déjà `effectiveness_verified`, le CONSTAT, mais rien ne distinguait un dossier qu'on a DÉLIBÉRÉMENT jugé non vérifiable d'un dossier qu'on a OUBLIÉ de vérifier — première question d'un auditeur, et réponse sans valeur si elle est reconstituée après coup. `verification_required` est donc **nullable** (V132) et l'état a **trois** valeurs : `NULL` = pas tranché, `FALSE` = décidé non requis, et dire « non » est une décision ; un `NOT NULL DEFAULT FALSE` aurait rempli la base d'un refus que personne n'a prononcé. Les trois gardes sont au SERVICE et non en `CHECK` (422) : exiger sans désigner, désigner sans exiger, et « ne plus exiger » qui efface vérificateur et consignes ; une mise à jour partielle ne décide rien. Le nom du vérificateur est RECOPIÉ à côté de son identifiant — un dossier se relit des années après, le compte aura pu disparaître (même parti que l'ADR 0071) — et un annuaire muet se DIT au lieu de se déguiser en liste vide. Index PARTIEL sur la seule lecture fréquente, la liste de travail du vérificateur (§4.2, §18.2 #2, §20) **Amende 18/09** : la question devient OBLIGATOIRE dans le popup d'action ouvert depuis une non-conformite — c'est le seul chemin ou elle se pose (le dossier vient peut-etre d'etre cree par ce meme geste), et le moment ou elle a un sens. L'etat a trois valeurs n'est pas remis en cause : l'obligation vit dans CE formulaire, pas dans le modele. Absente du meme popup ouvert depuis une fiche CAPA, ou « Modifier » la regle deja. La verification part AVANT l'action : elle porte les gardes, et l'inverse laisserait une action que l'utilisateur doublerait en reessayant | Accepté |
| [0074](./0074-l-action-passe-avant-le-dossier-qui-la-range.md) | **L'action passe avant le dossier qui la range.** Depuis une non-conformite, « Escalader CAPA » creait un dossier VIDE qu'il fallait ensuite retrouver pour y ecrire ce qu'on allait faire — l'inverse de l'ordre mental, ou l'on a en tete l'action et non la structure qui la rangera. Le bouton devient « Ajouter une action » et ouvre LE MEME formulaire que dans une CAPA (`CapaActionDialogModule`, sorti a part pour que deux modules l'ouvrent sans greffer les routes /capa sous /nc) ; le dossier se cree en chemin s'il manque, et le bouton n'est plus barre quand il existe — on ajoute une deuxieme action, une troisieme. **Revise l'ADR 0072 §1** : `NEW_CUSTOMER` nommait un contexte COMMERCIAL et n'apprenait rien a qui filtrait sa liste ; il devient `MAJOR_MODIFICATION`, le cas le plus frequent apres le NPI, qui tombait dans `OTHER`. La V133 depose la contrainte, CONVERTIT les projets (jamais ne les supprime : ils portent cycle, phases, livrables et pieces), puis la repose — l'ordre inverse echouerait sur la premiere ligne et arreterait la base entiere. Le retour d'un Ishikawa ramene a la NC d'origine et non a la liste ; la CAPA rejoint le groupe « Non-conformite » ; le « Projet par defaut » de la V131 passe a l'anglais, mais SEULEMENT s'il est reste intact (ADR 0070). Selecteur de date Material sur l'echeance APQP, avec conversion par composantes LOCALES : `toISOString()` enregistrerait l'echeance la veille a l'est de Greenwich (§3.6, §4.2, §15.1) | Accepté |
