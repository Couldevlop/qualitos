...
| A05 | Security Misconfiguration | ✅ | Images minimales sans shell ni gestionnaire de paquets, en `nonroot` : `gcr.io/distroless/java21-debian12:nonroot` (Java) et `cgr.dev/chainguard/python` (Python — bascule du 2026-08-02, cf. [ADR 0047](../adr/0047-base-python-chainguard.md) : la base distroless Python n'était pas reconstruite assez souvent et accumulait des CVE non patchables). Actuator restreint à `/health` + `/info`. CSP via OWASP Secure Headers filter. Whitelabel error page désactivée. |
