...
| **IoT protocoles terrain** (OPC-UA/MQTT/HL7/LoRaWAN/Sparkplug…) | ~~STUB~~ → **MQTT + OPC-UA + HL7 FHIR + LoRaWAN + Sparkplug B RÉELS** | ✅ Connecteurs MQTT (Paho v5), OPC-UA (Milo 0.6.16), HL7 FHIR R5 (REST entrant), **LoRaWAN** (ADR 0026 : TTN/ChirpStack, décodeur pur `{deviceName, decoded{…}, units{…}, time}`, `POST /api/v1/iot/lorawan/uplink`) et **Sparkplug B** (ADR 0027 : décodeur pur JSON NDATA/DDATA + handler `Outcome`, `POST /api/v1/iot/sparkplug`). Tous deux : tenant via `findUniqueByCode` **fail-closed** cross-tenant (chemin MQTT, §18.2 #2), gating off par défaut. Décodage protobuf brut (Eclipse Tahu) délégué. Reste STUB : Modbus, DICOM. |
...
5. ~~**Connecteurs protocoles IoT**~~ → ✅ **MQTT + OPC-UA + HL7 FHIR + LoRaWAN livrés** (Paho v5, Milo 0.6.16, FHIR R5 REST, LoRaWAN TTN/ChirpStack webhook ; tenant depuis le registre device, fail-closed). Reste : Sparkplug B / Modbus / DICOM.
