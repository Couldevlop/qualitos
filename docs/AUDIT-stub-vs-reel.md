...
| **NLP réclamations (sentiment + classification)** | ~~ABSENT~~ → **RÉEL (lexical)** | ✅ `domain/service/complaint_nlp.py` : sentiment lexical pondéré (négation, intensifieurs) + classification par termes-graines + détection critique, pur FR/EN. `POST /v1/ai/complaints/analyze` + relais engine `complaintnlp/` + UI `/complaints-nlp`. **BERT/Whisper pleins** (transformers/torch + transcription audio) restent un lot GPU derrière le même contrat. |
