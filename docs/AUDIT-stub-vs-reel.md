...
| **IoT protocoles terrain** (OPC-UA/MQTT/HL7/LoRaWAN…) | ~~STUB~~ → **MQTT + OPC-UA + HL7 FHIR + LoRaWAN RÉELS** | ✅ Connecteurs MQTT (Paho v5), OPC-UA (Milo 0.6.16), HL7 FHIR R5 (endpoint REST) et **LoRaWAN (TTN/ChirpStack, endpoint webhook `POST /api/v1/iot/lorawan/uplink`)** livrés dans api-iot-hub. LoRaWAN : décodeur pur d'uplinks décodés `{deviceName, decoded{…}, units{…}, time}` (champ device configurable devEUI/deviceName), handler tenant-agnostique via `findUniqueByCode` **fail-closed** sur collision cross-tenant (chemin MQTT, §18.2 #2), gating off par défaut (`qualitos.iot.lorawan.enabled`). Reste STUB : Sparkplug B, Modbus, DICOM. |
