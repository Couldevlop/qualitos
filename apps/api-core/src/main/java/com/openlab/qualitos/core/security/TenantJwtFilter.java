...
/**
 * Filtre Spring qui extrait le claim {@code tenant_id} depuis le JWT Bearer token
 * et le stocke dans {@link TenantContext} pour la durée de la requête.
 *
 * <p>Règle absolue : le tenant_id n'est JAMAIS lu depuis le body, les query params
 * ou les headers custom — uniquement depuis le claim JWT validé par Spring Security.
 *
 * <p>Le contexte est toujours nettoyé dans le bloc finally pour éviter toute fuite
 * de tenant entre threads réutilisés (pool de threads Tomcat).
 *
 * <p>Enregistré comme @Bean dans {@link com.openlab.qualitos.core.config.SecurityConfig}
 * (pas de @Component) pour un contrôle explicite sur son ordre et son inclusion dans les tests.
 */
