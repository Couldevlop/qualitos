 ...
 * lui-même.
 *
 * <p><b>L'acteur vient du jeton, jamais du corps.</b> {@code api-core} n'a pas
 * (encore) d'équivalent au {@code CurrentUser} du moteur de qualité ; on
 * s'appuie donc directement sur {@link Authentication#getName()}, qui pour un
 * jeton JWT Keycloak ({@code JwtAuthenticationToken}) renvoie le claim
 * {@code sub} — l'identifiant, stable et non falsifiable côté client, de
 * l'utilisateur authentifié. Ajouter un champ {@code updatedBy} à
 * {@link ModulePriceDto.SaveCommand} aurait permis à n'importe quel
 * SUPER_ADMIN de signer un changement de tarif au nom d'un autre : même
 * principe que le refus, dans {@link BillingProfileController}, de lire le
 * client facturé depuis le corps plutôt que depuis le chemin.
 */
@RestController
