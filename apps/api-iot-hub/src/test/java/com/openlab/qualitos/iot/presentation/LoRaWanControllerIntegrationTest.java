...
/**
 * Intégration LoRaWAN → télémétrie via le contexte complet (H2, sécurité réelle).
 *
 ...
 * peut quand même pousser l'uplink (rôle GATEWAY) — la mesure est attribuée au tenant
 * propriétaire du device.
 */
@SpringBootTest(properties = "qualitos.iot.lorawan.enabled=true")
@ActiveProfiles("test")
@Tag("web")
@Import(LoRaWanControllerIntegrationTest.StubExternalsConfig.class)
class LoRaWanControllerIntegrationTest {
  ...
  private static String uplink(String deviceCode) {
    return """
        {"deviceName":"%s",
  ...
  private void registerDevice(UUID tenant, String code) throws Exception {
    DeviceDtos.RegisterDeviceRequest body = new DeviceDtos.RegisterDeviceRequest(
        code, "Capteur sol LoRa", DeviceType.AGRO_STATION, Protocol.LORAWAN,
        "Exploitation", "Parcelle-12", null, null, null, null, null);
  ...
  @Test
  void unauthenticatedUplinkRejected() throws Exception {
    mvc().perform(post("/api/v1/iot/lorawan/uplink")
            .contentType(MediaType.APPLICATION_JSON)
            .content(uplink("SOL-LORA-01")))
        .andExpect(status().isUnauthorized());
  }

  @Test
  void uplinkIsIngestedForKnownDevice() throws Exception {
    UUID tenant = UUID.randomUUID();
    registerDevice(tenant, "SOL-LORA-02");

    mvc().perform(post("/api/v1/iot/lorawan/uplink")
            .with(jwt().jwt(b -> b.claim("tenant_id", tenant.toString()))
                .authorities(new SimpleGrantedAuthority("ROLE_GATEWAY")))
            .contentType(MediaType.APPLICATION_JSON)
            .content(uplink("SOL-LORA-02")))
  ...
  void crossTenantCollisionFailsClosed() throws Exception {
    ...
    registerDevice(owner, "DUP-LORA");
    registerDevice(other, "DUP-LORA");

    mvc().perform(post("/api/v1/iot/lorawan/uplink")
            .with(jwt().jwt(b -> b.claim("tenant_id", owner.toString()))
                .authorities(new SimpleGrantedAuthority("ROLE_GATEWAY")))
            .contentType(MediaType.APPLICATION_JSON)
            .content(uplink("DUP-LORA")))
        .andExpect(status().isUnprocessableEntity())
        .andExpect(jsonPath("$.ingested").value(0));
  }

  @Test
  void noNumericMeasurementIs422() throws Exception {
    UUID tenant = UUID.randomUUID();
    registerDevice(tenant, "SOL-LORA-05");
    ...
    mvc().perform(post("/api/v1/iot/lorawan/uplink")
            ...
            .contentType(MediaType.APPLICATION_JSON)
            .content("""
                {"deviceName":"SOL-LORA-05","decoded":{"label":"x"}}"""))
        .andExpect(status().isUnprocessableEntity())
        .andExpect(jsonPath("$.ingested").value(0));
  }
