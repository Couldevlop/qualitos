...
from domain.service import isolation_forest, reconstruction
...
class AnomalyDetectUseCase:
    ...
    def execute(
        ...
        *,
        ...
        matrix = self._to_matrix(samples)
    ...
    def _to_matrix(self, samples: list[list[float]]) -> np.ndarray:
        """Valide la forme (lignes de même longueur, bornes) et renvoie la matrice float."""
        width = len(samples[0])
        if width == 0:
