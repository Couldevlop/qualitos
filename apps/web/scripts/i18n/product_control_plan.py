...
TRANSLATIONS = {
    ...
    'controlplan.section-where': ('Où, dans la gamme', 'Where, in the routing', 'Dónde, en la ruta', 'أين، ضمن المسار', '工程のどこで', '在工艺流程的何处'),
    'controlplan.section-what': ('Ce qui est contrôlé', 'What is checked', 'Qué se controla', 'ما يتم فحصه', '何を管理するか', '控制什么'),
    'controlplan.section-how': ('Comment on le mesure', 'How it is measured', 'Cómo se mide', 'كيف يُقاس', 'どう測定するか', '如何测量'),
    'controlplan.section-reaction': ('Si le contrôle sort des clous', 'If the check goes out of bounds', 'Si el control se sale de límites', 'إذا خرج الفحص عن الحدود', '管理が外れた場合', '若控制超出范围'),
    'controlplan.section-why': ('Pourquoi ce contrôle existe', 'Why this check exists', 'Por qué existe este control', 'لماذا يوجد هذا الفحص', 'この管理が存在する理由', '该控制存在的原因'),
    'controlplan.blocked-characteristic': ('Indiquez ce qui est contrôlé pour pouvoir enregistrer.', 'State what is checked to save.', 'Indique qué se controla para poder guardar.', 'حدّد ما يتم فحصه لتتمكن من الحفظ.', '保存するには管理対象を入力してください。', '填写控制对象后方可保存。'),
